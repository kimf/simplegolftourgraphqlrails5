...
Rails.application.config.sorcery.submodules = [:remember_me, :reset_password, :activity_logging, :user_activation, :external]
...
Rails.application.config.sorcery.configure do |config|
  ...
  # Default: `nil`
  #
  config.cookie_domain = 'simplegolftour.com'
  ...
  #
  config.facebook.key = "681890295166727"
  config.facebook.secret = "c6d54aa264a8b00533fba87a0571f6c2"
  ...
  module Model
    module InstanceMethods
      def generic_send_email(method, mailer)
        mailer = sorcery_config.send(mailer).to_s
        method = sorcery_config.send(method)

        if %w(activation_needed_email activation_success_email welcome_from_facebook_email).include?(method.to_s)
          SorceryMailJob.new.async.later(1800, mailer, method, self)
        else
          SorceryMailJob.new.async.perform(mailer, method, self)
        end
      end
    end
  end
